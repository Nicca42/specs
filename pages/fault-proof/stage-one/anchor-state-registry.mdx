...
created in the same block _after_ the Retirement Timestamp was set will be considered Retired Games
...
brevity. A Dispute Game can go from being a Proper Game to later _not_ being an **Improper Game**
...
mechanism is active. However, this is a _temporary_ condition such that Registered Games that are
...
_temporarily_ prevent Dispute Games from being used by consumers like the `OptimismPortal` while
